...
class Main(QtCore.QThread):
    ...
    def __init__(self, parent=None):
        ...
        self.dialog = theard_dialog.theard_dialog()
    ...
    def start_gtp(self):
        ...
        window.setWindowTitle("AutoGO - 空闲")
        QtWidgets.QApplication.processEvents()
    ...
    def start_listen(self):
        ...
        self.click = v_click.SimulatedMouse(self.w_hwnd)
    ...
    def run(self):
        while True:
            ...
            if not self.gb_cv.update_board(w_img):
                if self.has_board:
                    self.stop_listen()
                    self.dialog.information(window,"提示","棋局结束或出现提示，已自动关闭监听")
                    return
                continue
            change = False
            f_change = False
            if not self.has_board:
                self.has_board = True
                result = go_dialog.GoDialog.get_color_threadsafe(window, w_img)
                self.color = result["my_color"]
                if result["my_color"] == result["current_turn"]:
                    f_change = True
                    self.zb = False
                else:
                    self.zb = True
            ...
            if self.zb:
                self.zb = False
                continue
            if self.laste_x != -1 and self.laste_y != -1:
                board.set_stone(self.laste_x,self.laste_y,None)
                self.laste_x,self.laste_y = -1,-1
            ...
            window.setWindowTitle("AutoGO - 思考中🤔")
            x,y = move = self.go_engine.get_ai_recommendation(self.color)
            window.setWindowTitle("AutoGO - 空闲")
            
            if not self.gb_cv.update_board(self.capture.capture()):
                if has_board:
                    self.stop_listen()
                    self.dialog.information(window,"提示","棋局结束或出现提示，已自动关闭监听")
                    return
                continue
            ch,_ = self.gb_cv.recognize()
            for i in range(0,19):
                for j in range(0,19):
                    if a[i][j] != ch[i][j]:
                        break
            ...
            if auto_play_sc.isChecked():
                p_x,p_y = self.gb_cv.get_stone_window_coord(x,y)
                self.click.SimulatedMouse("left",(p_x,p_y))
            else:
                self.laste_x,self.laste_y = x,y
                board.set_stone(x,y,{"color":"#00FF00","label":""})
