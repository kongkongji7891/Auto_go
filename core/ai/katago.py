...
class KataGoGTP:
    ...
    def play_move(self, row: int, col: int, color: str) -> bool:
        ...
        :param row: 0~18
        :param col: 0~18
        :param color: "black"/"white" 或 "b"/"w"，也可以是整数 1/2
        :return: 是否成功
        """
    ...
    def get_ai_recommendation(self, color: str) -> Optional[Tuple[int, int]]:
        ...
        :param color: 当前轮到哪一方（字符串或整数 1/2）
    ...
    def undo_move(self) -> bool:
        """
        撤销上一步落子（通过 GTP undo 命令）。
        :return: 是否成功撤销
        """
        ok, msg = self._execute_task("undo")
        self._last_command_ok = ok
        if not ok:
            logger.warning(f"撤销失败: {msg}")
        return ok
    ...
    @staticmethod
    def _color_full(color) -> str:
        """
        将颜色参数标准化为 full name（支持 int 1/2 或 string）
        1 → "black", 2 → "white", 其他字符串按原映射处理
        """
